...
function _getScale(element) {
    try {
        ...
        if (transform === 'none' || !transform) return { scaleX: 1, scaleY: 1 };
...
var hiprint = function (t) {
    ...
    var o = function () {
        ...
        return t.prototype.setDefault = function (t) {
            ...
            return Object.keys(this).filter(function (t) {
                ...
                if ("number" != typeof e[n] && "string" != typeof e[n] && _typeof(e[n]) != _typeof(!0) && n != "testData" || (t[n] = e[n]), "style" == n) { // testData的处理解决复制模板同时复制数据
                    t.style = {};
                    var i = e[n];
    ...
    var _entity_PrintElementEntity__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(17),
        ...
        BasePrintElement = function () {
            ...
            return BasePrintElement.prototype.getConfigOptionsByName = function (t) {
                ...
                if (e._editing) {
                    ...
                    if (t.startsWith(title) && e.options.field) {
                        if (t.length > title.length) {
                            var res = t.indexOf("：") > -1; // 如果标题没有冒号(：), 就不分割
                            e.options.testData = res ? t.split("：")[1] : t;
                        } else {
                            e.options.title = t;
                            e.options.testData = "";
                        }
                ...
                this.designTarget?.hidraggable('update', { draggable: this.options.draggable });// 更新拖拽设置
				...
				var type = this.options.textType
				if(type==='oval') return ['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w']
        		return ["se", "s", "e"];
                ...
                onStopResize: function onStopResize() {
                    hinnn.event.trigger("hiprintTemplateDataChanged_" + n.templateId, "大小");
    ...
    var _ReconsitutionTableColumns__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(19),
        ...
        TableExcelHelper = function () {
            ...
            return TableExcelHelper.createTableHead = function (t, e) {
                ...
                return typeof p == "function" && a.append(p(n, e, o, r)), a;
                ...
                (e || (e = []), i.groupFields.length) ? _assets_plugins_hinnn__WEBPACK_IMPORTED_MODULE_1__.a.groupBy(e, i.groupFields, function (t) {
                    ...
                    if (t.rows.forEach(function (t, rowIndex) {
                        var e = TableExcelHelper.createRowTarget(o, t, n, i, rowIndex, t.rows);
                        r.append(e);
                    ...
                    var e = TableExcelHelper.createRowTarget(o, t, n, i, rowIndex, e);
                    r.append(e);
                });
                return r;
            },TableExcelHelper.createRowTarget = function (t, e, n, i, rowIndex, tableData) {
                var o = $("<tr></tr>");
                o.data("rowData", e);
                var mergeMap = {};
                ...
                t.rowColumns.forEach(function (t, colIndex) {
                    ...
                    if (n.rowsColumnsMerge) {
                        try { eval('rowsColumnsMerge=' + n.rowsColumnsMerge); } catch (error) {}
                    }
                    var mergeRes = defMerge;
                    if(rowsColumnsMerge){
                        rowsColumnsMerge = rowsColumnsMerge(e, t, rowIndex, colIndex, n.testData);
                        mergeRes = Object.assign({}, defMerge, rowsColumnsMerge);
                    } 
                    mergeMap[colIndex] = mergeRes;
                    if (mergeRes.rowspan > 1 || mergeRes.colspan > 1) {
                        for (var r = rowIndex; r < rowIndex + mergeRes.rowspan; r++) {
                            for (var c = colIndex; c < colIndex + mergeRes.colspan; c++) {
                                if (r !== rowIndex || c !== colIndex) {
                                    if (!window._mergeSkipMap) window._mergeSkipMap = {};
                                    window._mergeSkipMap[r + "-" + c] = true;// 标记合并
                                }
                            }
                        }
                    }
                ...
                t.rowColumns.forEach(function (t, colIndex) {
                    ...
                    if (mergeRes.rowspan > 1 || mergeRes.colspan > 1) {
                        r.attr({ "rowspan": mergeRes.rowspan, "colspan": mergeRes.colspan });
                    }
                    if (mergeRes.tdStyle) r.css(mergeRes.tdStyle); // 设置td样式
                    if (mergeRes.tdClass)  r.addClass(mergeRes.tdClass); 
                    if(!t.field && t.columnId) t.field = t.columnId; // 表格列未配置field，使用columnId
                    t.field && r.attr("field", t.field), t.align && r.css("text-align", t.align), t.vAlign && r.css("vertical-align", t.vAlign);
                    var val = e[t.field]; // 单元格数据
                    var a = TableExcelHelper.getColumnFormatter(t),
                        p = a ? a(val, e, colIndex, n) : val;
                    var result = TableExcelHelper.cellFormatter(val, e, t, rowIndex, colIndex, n);
                    if(val && result) p = result;
                    r.html(p);
                    var s = TableExcelHelper.getColumnStyler(t);

                    if (s) {
                        var l = s(val, e, colIndex, n);
                ...
                var rowStyleSet = false;
                t.rowColumns.forEach(function (t, colIndex) {
                    if (rowStyleSet) return;
                    var mergeRes = mergeMap[colIndex];
                    if (mergeRes && (mergeRes.trStyle || mergeRes.trClass)) {
                        if (mergeRes.trStyle) o.css(mergeRes.trStyle); // 设置tr样式
                        if (mergeRes.trClass) o.addClass(mergeRes.trClass); 
                        rowStyleSet = true;
                    }
                });
				...
				for (let i = 0; i < t.totalLayer; i++) {
					...
					t[i].forEach((column, columnIdx) => {
						newColumns[i].push(...new Array(column.colspan).fill({ ...column, colspan: 1 })); // 创建 colspan 个 column
					});
				}
            ...
            }, TableExcelHelper.cellFormatter = function (value, row, column, rowIndex, colIndex, n) {
				var cellFormatter = void 0;
                if (n.cellFormatter) try {
                    var s = "cellFormatter=" + n.cellFormatter;
                    eval(s);
                } catch (t) {
                    console.log(t);
                }
                return cellFormatter ? cellFormatter({value, row, column, rowIndex, colIndex, testData:n.testData}) : value;
            }, TableExcelHelper;
        }();
}, function (t, e, n) {
    "use strict";
    ...
    var i = function () {
        ...
        return t.prototype.css = function (t, e) {
            ...
            return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">字体行高</div>  <div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
        }, t.prototype.getValue = function () {
        ...
        r = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">字体大小</div>  <div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        h = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">表头行高</div>  <div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                    var t = this.target.find("input").val();
                    if (t) return parseFloat(t.toString());
                }, t.prototype.setValue = function (t) {
        ...
        f = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">表头字体大小</div>  <div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        v = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">表体行高</div>  <div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                    var t = this.target.find("input").val();
                    if (t) return parseFloat(t.toString());
                }, t.prototype.setValue = function (t) {
        ...
        b = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(` <div class="hiprint-option-item"><div class="hiprint-option-item-label">边框大小</div><div class="hiprint-option-item-field"><input type="number" min="0" step="0.5" class="auto-submit" /></div></div>`), this.target;
            }, t.prototype.css = function (t, e) {
                if (t && t.length) {
        ...
        qrCodeLevel = function () {
            ...
            return t.prototype.createTarget = function () {
				...
				const levelOpts = LevelOptions.map(item => `<option value="${item.value}" title="${item.value}">${item.name}</option>`).join('');
                return this.target = $(`<div class="hiprint-option-item"><div class="hiprint-option-item-label">二维码容错率</div><div class="hiprint-option-item-field"><select class="auto-submit">${levelOpts} </select></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("select").val();
                return parseInt(t || 0);
            }, t.prototype.setValue = function (t) {
        ...
        w = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        标题\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:50px;resize:vertical" placeholder="请输入标题" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
        ...
        imageFit = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return null;
					...
					{ name: "剪裁", value: "cover" },
					{ name: "填充", value: "fill" },
					{ name: "原始尺寸", value: "none" }
				];
				const fitOpts = fitOptions.map(item => `<option value="${item.value}" title="${item.value}">${item.name}</option>`).join('');
                this.target = $(`<div class="hiprint-option-item hiprint-option-item-row">
                        <div class="hiprint-option-item-label">缩放类型</div>
                        <div class="hiprint-option-item-field"><select class="auto-submit">${fitOpts} </select></div>
                    </div>`), this.target;
                return this.target;
            }, t.prototype.getValue = function () {
        ...
        borderRadius = function () {
            ...
            return t.prototype.css = function (t, e) {
                if (t && t.length) {
                    if (e) return t.css('border-radius', e + "%");
                }
                return null;
            }, t.prototype.createTarget = function () {
                return this.target = $(`<div class="hiprint-option-item">
                    <div class="hiprint-option-item-label">边框圆角</div>
                    <div class="hiprint-option-item-field"><input type="number" min="0" max="50" class="auto-submit" /></div>
                </div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return t;
        ...
        PaperNumberTop = function () {
            function t() {
                this.name = "paperNumberTop";
            }
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        顶部偏移\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" min="0" step="0.5" placeholder="页码顶部偏移(pt)" class="auto-submit">\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        PaperNumberLeft = function () {
            function t() {
                this.name = "paperNumberLeft";
            }
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        左侧偏移\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" min="0" step="0.5" placeholder="页码左侧偏移(pt)" class="auto-submit">\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        N = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        左偏移\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" min="0" step="0.5" placeholder="偏移量pt" class="auto-submit" >\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        U = function () {
            ...
            return t.prototype.css = function (t, e) {
                ...
                return this.target = $('<div class="hiprint-option-item">\n        <div class="hiprint-option-item-label">\n        旋转角度\n        </div>\n        <div class="hiprint-option-item-field">\n        <input type="number" step="1" class="auto-submit"/>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("input").val();
                if (t) return parseFloat(t.toString());
            }, t.prototype.setValue = function (t) {
        ...
        K = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        边框设置\n        </div> <div class="hiprint-option-item-field"></div> </div>'), this.target;
        ...
        lockWidthHeight = function () {
            function t() {
                this.name = "lockWidthHeight";
            }
            return t.prototype.css = function (t, e) { 
                if (t && t.length) { 
                    const minDis = Math.min(t.outerWidth(), t.outerHeight()) + "px"; 
                    if (e) return t.css({"width":minDis,"height":minDis}), `width:${minDis};height:${minDis};`;
                }
                return null;
            }, t.prototype.createTarget = function () {
                return this.target = $(` <div class="hiprint-option-item">
                        <div class="hiprint-option-item-label">锁定宽高</div>
                        <div class="hiprint-option-item-field">
                            <input style="width:18px;height:18px;" type="checkbox" class="auto-submit">    
                        </div>
                    </div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find('input').is(':checked');
                if (t) return t;
            }, t.prototype.setValue = function (t) { 
                if (t) this.target.find("input").attr("checked", "checked");
                else this.target.find("input").removeAttr("checked");
            }, t.prototype.destroy = function () {
                this.target.remove();
            }, t;
        }(),
        ...
        TableMerge = function () {
            ...
            return t.prototype.createTarget = function () {
                ...
                return this.target = $(`<div class="hiprint-option-item">
                    <div class="hiprint-option-item-label">跨页合并</div>
                    <div class="hiprint-option-item-field">
                        <select class="auto-submit">
        ...
        ft = function () {
            ...
            return t.prototype.createTarget = function (t, e, n) {
                ...
                fileInput.onchange = function (event) {
                    ...
                    _blobToDataURL(blob, (err, dataUrl) => {
                        if(err) return
                        const $input = this.target.find('textarea');
                        // const dataUrl = URL.createObjectURL(file); (或使用blob:url)
                        $input.val(`function (src, options){\n  return "${dataUrl}"\n}`);
                        $input.trigger('input').trigger('change').trigger('blur');
                        document.body.removeChild(fileInput);
                    })
                }.bind(this);
        ...
        gt = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="styler">样式函数</div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function (value, options, target, templateData) {\n  return { color: \'red\' }\n}" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        rowcolumns = function () {
            ...
            var placeholder = `function(row,colum,rowIndex,colIndex,tableData){\n  if(rowIndex===1&&colIndex===1) {\n    return {\n      rowspan: 2, colspan: 1,\n      trStyle: {color: 'red'},\n      tdStyle: {color: 'gray'},\n      trClass: 'xxx',\n      tdClass: 'xxx',\n    };\n  }\n}`;
            return t.prototype.createTarget = function () {
                return this.target = $(`<div class="hiprint-option-item hiprint-option-item-row vert"><div class="hiprint-option-item-label" title="rowsColumnsMerge: ${placeholder}">行/列合并函数</div><div class="hiprint-option-item-field"><textarea style="height:100px;" placeholder="${placeholder}" class="auto-submit"></textarea></div></div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        cellFormatter = function () {
            function t() {
                this.name = "cellFormatter";
            }

            return t.prototype.createTarget = function () {
                return this.target = $(` <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="cellFormatter">\n        格式化函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(options){\n  return options.value;\n}" class="auto-submit"></textarea>\n        </div>\n    </div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        mt = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="footerFormatter">\n        表格脚函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(options,rows,testData,gridRowsData){\n  return \'<tr></tr>\';\n}" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        vt = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="gridColumnsFooterFormatter">\n        多组表格脚函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(options,rows,testData,gridRowsData){\n return \'<div>表格底部</div>\';\n}" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        yt = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(` <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="rowStyler">\n        行样式函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(value,options){\n  return { color:'red' };\n}" class="auto-submit"></textarea>\n        </div>\n    </div>`), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        Pt = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="styler2">\n        样式函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(value,row,index,options){\n  return {color:\'red\'};\n}" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        _t = function () {
            ...
            return t.prototype.createTarget = function () {
                return this.target = $(' <div class="hiprint-option-item hiprint-option-item-row vert">\n        <div class="hiprint-option-item-label" title="formatter2">\n        单元格格式化函数\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;resize:vertical" placeholder="function(value,row,index,options){\n  return value;\n}" class="auto-submit"></textarea>\n        </div>\n    </div>'), this.target;
            }, t.prototype.getValue = function () {
                var t = this.target.find("textarea").val();
                if (t) return t;
            }, t.prototype.setValue = function (t) {
        ...
        this.options = n.data(t.target, "hireizeable").options, this.init(t.target);
            ...
            bindResizeEvent: function bindResizeEvent(t, e) {
                ...
                t.on("mousedown", function (t) {
                    ...
                    if ((d || c || h || f || g || m || y || v || b)) {
						i.options.onStopResize();
					}
					d = !1, c = !1, h = !1, f = !1, g = !1, m = !1, y = !1, v = !1, b = !1;
				});
            },
    ...
    var _r,
        ...
        T = function () {
            ...
            function t(t={}, u, d) {
                this.gridLine = t.gridLine,
                this.watermarkOptions = t.watermarkOptions,
                this.panelIdx = t.index,
                this.defaultPaperNumberFormat = "paperNo-paperCount", 
                this.printLine = 0, 
                this.templateId = t.templateId, 
                this.width = o.a.mm.toPt(t.width), 
                this.height = o.a.mm.toPt(t.height), 
                this.mmwidth = t.width, 
                this.mmheight = t.height, 
                this.paperHeader = t.paperHeader, 
                this.paperFooter = t.paperFooter, 
                this.contentHeight = t.paperFooter - t.paperHeader, 
                this.createTarget(), 
                this.index = u, 
                this.paperNumberLeft = t.paperNumberLeft || parseInt((this.width - 30).toString()), 
                this.paperNumberTop = t.paperNumberTop || parseInt((this.height - 22).toString()), 
                this.paperNumberDisabled = t.paperNumberDisabled, 
                this.paperNumberFormat = t.paperNumberFormat, 
                this.referenceElement = d ? $.extend({}, d) : new E.a({
                    top: 0,
                    left: 0,
                    height: 0,
                    width: 0,
            ...
            return t.prototype.subscribePaperBaseInfoChanged = function (t) {
                ...
                var e = this;
                this.createHeaderLine(), this.createFooterLine(), this.target.addClass(`design ${this.gridLine ? "grid" : ""}`), this.paperNumberTarget = this.createPaperNumber(this.formatPaperNumber(1, 1)), this.createRuler(), this.createWaterMark(true, this.panelIdx, this.watermarkOptions), this.resetPaperNumber(this.paperNumberTarget), $(this.paperNumberTarget).bind("dblclick.hiprint", function () {
                    ...
                    o.a.event.trigger("BuildCustomOptionSettingEventKey_" + e.templateId, {
                        options: {
                            paperNumberTop: e.paperNumberTop,
                            paperNumberLeft: e.paperNumberLeft,
            ...
            }, t.prototype.updatePaperNumberPosition = function ( ) {
                var e = this,
                n = this.target.find(".hiprint-paperNumber");
                if (n.length) return n.css("top", this.paperNumberTop + "pt"), n.css("left", this.paperNumberLeft + "pt"), n;
                ...
                this.paperFooter = this.paperFooter || this.height,
                this.footerLinetarget.css("top",this.paperFooter + "pt"),
                this.contentHeight = this.paperFooter - this.paperHeader;
                this.paperNumberLeft = this.paperNumberLeft || parseInt((this.width - 30).toString()),
                this.paperNumberTop = this.paperNumberTop || parseInt((this.height - 22).toString()),
              ...
              if (s) {
				this.scale = s;
				!p&&(p={x: 0 , y: 0, transition: 'none'}); 
				this.target.css({transform: `translate(${p.x || 0}px, ${p.y || 0}px) scale(${s || 1})`, transition: p.transition || 'transform 0.3s'});
                if (s > 1) {
                  this.target.css({transformOrigin: "-1.5% -1.07%"});
                } else {
                ...
                this.target.append(`<div class="hiprint_rul_wrapper">
                        <img class="h_img" src="./assets/image/h_img.svg" />
                        <img class="v_img" src="./assets/image/v_img.svg" />
                    </div>`);
                ...
                if (options.content) {
                    if (watch) {
                        window.destroyWatermark(Object.assign({}, options, {
                            id: `${e.templateId}_${e.panelIdx}_${idx || 1}_${e.index}`,
                        }));
                    }
                    window.createWatermark(options);
                } else {
                    window.destroyWatermark(options);
                }
            }, t.prototype.displayHeight = function () {
        ...
        D = function (t) {
            ...
            return H(e, t), e.prototype.getDesignTarget = function (e) {
                ...
                var title = this.options.title;
                var t = title == undefined ? this.printElementType.title : title || "";
        ...
        rt = function () {
            return function (t) {
                ...
                this.paperHeader = t.paperHeader || 0, this.paperFooter = t.paperFooter || o.a.mm.toPt(this.height), this.printElements = t.printElements || [], this.paperNumberLeft = t.paperNumberLeft, this.paperNumberTop = t.paperNumberTop, this.paperNumberDisabled = t.paperNumberDisabled, this.paperNumberFormat = t.paperNumberFormat, this.panelPaperRule = t.panelPaperRule, this.rotate = t.rotate || void 0, this.firstPaperFooter = t.firstPaperFooter, this.evenPaperFooter = t.evenPaperFooter, this.oddPaperFooter = t.oddPaperFooter, this.lastPaperFooter = t.lastPaperFooter, this.topOffset = t.topOffset, this.fontFamily = t.fontFamily, this.leftOffset = t.leftOffset, this.orient = t.orient, this.paperNumberTop = t.paperNumberTop, this.paperNumberLeft = t.paperNumberLeft, this.paperNumberContinue = t.paperNumberContinue, this.hostDomain = t.hostDomain, this.watermarkOptions= t.watermarkOptions;
            };
        }(),
        at = function () {
            function t(t, e, n, i) {
        ...
        pt = function () {
            function t(t, e) {
                ...
                this.templateId = e.id, this.index = t.index, this.width = t.width, this.height = t.height, this.paperType = t.paperType, this.paperHeader = t.paperHeader, this.paperFooter = t.paperFooter, this.initPrintElements(t.printElements), this.paperNumberLeft = t.paperNumberLeft, this.paperNumberTop = t.paperNumberTop, this.paperNumberDisabled = t.paperNumberDisabled, this.paperNumberFormat = t.paperNumberFormat, this.panelPaperRule = t.panelPaperRule, this.firstPaperFooter = t.firstPaperFooter, this.evenPaperFooter = t.evenPaperFooter, this.oddPaperFooter = t.oddPaperFooter, this.lastPaperFooter = t.lastPaperFooter, this.topOffset = t.topOffset, this.leftOffset = t.leftOffset, this.fontFamily = t.fontFamily, this.orient = t.orient, this.target = this.createTarget(), this.rotate = t.rotate, this.paperNumberTop = t.paperNumberTop, this.paperNumberLeft = t.paperNumberLeft, this.paperNumberContinue = t.paperNumberContinue, this.hostDomain = t.hostDomain, this.watermarkOptions = t.watermarkOptions || {};
            }
            ...
            return t.prototype.design = function (t) {
                ...
                this.orderPrintElements(), this.designPaper = this.createNewPage(0), this.target.html(""), this.target.append(this.designPaper.getTarget()), this.droppablePaper(this.designPaper), this.designPaper.design(t), this.designPaper.subscribePaperBaseInfoChanged(function (t) {
                    ...
                    o.a.event.trigger("BuildCustomOptionSettingEventKey_" + e.templateId, {
                        ...
                        callback: function callback(t) {
                            if(t.paperNumberLeft !== e.paperNumberLeft || t.paperNumberTop !== e.paperNumberTop){
                                e.designPaper.updatePaperNumberPosition(t);
                            ...
                            e.panelLayoutOptions = t.panelLayoutOptions || {}, e.paperNumberTop = t.paperNumberTop, e.paperNumberLeft = t.paperNumberLeft, e.designPaper.paperNumberTop = t.paperNumberTop, e.designPaper.paperNumberLeft = t.paperNumberLeft, e.paperNumberContinue = t.paperNumberContinue, e.hostDomain = t.hostDomain,
            ...
            }, t.prototype.zoom = function (s, p, d) { 
                if (p) {
                    this.scale = s
                } else {
                    this.scale = void 0
                }
                this.designPaper.zoom(s, d);
            }, t.prototype.getTarget = function () {
                return this.target;
                ...
                var height = t.maxY - t.minY + scrollH * sign;
                var width = t.maxX - t.minX + scrollW * sign;
            ...
            return t.prototype.init = function () { },
                t.prototype.clearSettingContainer = function () {
                    ...
                    i.getPrintElementOptionItems().forEach(function (t) {
                        ...
                        t.setValue(i.options[t.name] ?? i.printElementType[t.name], i.options, i.printElementType);
                    });
                    ...
                    }), r.find(".auto-submit").on("change input", function (t) {
                        i.submitOption();
                    ...
                    var r = $('<div class="hiprint-option-items"></div>');
                    t.title && r.append(`<div class="hiprint-option-item hiprint-option-item-row"><div class="hiprint-option-item-label hiprint-option-title">${t.title}</div><div class="hiprint-option-item-field"></div></div>`), o.forEach(function (e) {
                        e.submit = function (e) {
                    ...
                    r.append(a), a.bind("click.submitOption", function () {
                        t.callback(n.getValueByOptionItems(o));
                    }), r.find(".auto-submit").on("change input", function (e) {
                        t.callback(n.getValueByOptionItems(o));
        ...
        dt = function () {
            ...
            return t.prototype.buildPagination = function (t) {
                ...
                for (var i = $('<ul class="hiprint-pagination"></ul>'), o = function o() {
                    var t = r,
                        isA = t===0 ? "selected" : "",
                        e = $(`<li class='${isA}'><span>页面${t + 1}</span>${t===0 ? "" : '<a href="javascript:void(0);">×</a>'}</li>`);
        ...
        ct = function () {
            function t(t) {
                ...
                var n = t || {};
                this.printPanels = [];
                this.dataMode = n.dataMode || !0;
                this.onDataChange = n.onDataChange;
                this.layerOption = n.layerOption;
            ...
            return t.prototype.design = function (t, e) {
            ...
            }, t.prototype.applyContinuousPaperNumbersDom = function (t) {
                if (!t || !t.length || !this.printPanels.every(function (t) {
                    return t.paperNumberContinue !== false;
                })) return;
                var e = this,
                    n = t.length && t.first().hasClass("hiprint-printPaper") ? t : t.find(".hiprint-printPaper"),
                    i = n.length;
                if (i < 1) return;
                n.each(function (t, n) {
                    var o = $(n).closest(".hiprint-printPanel"),
                        r = (o.attr("class") || "").match(/panel-index-(\d+)/),
                        a = r ? parseInt(r[1], 10) : 0,
                        p = e.printPanels[a] || e.printPanels[0],
                        s = p && p.paperNumberFormat ? p.paperNumberFormat : "paperNo-paperCount";
                    if (p && p.paperNumberDisabled) return;
                    var l = $(n).find(".hiprint-paperNumber");
                    l.length && l.html(s.replace("paperNo", (t + 1).toString()).replace("paperCount", i.toString()));
                });
                ...
                return n.applyContinuousPaperNumbersDom(i), e && e.imgToBase64 && this.transformImg(i.find("img")), i;
                ...
                }), this.applyContinuousPaperNumbersDom(i), i;
            ...
            }, t.prototype.zoom = function (s, p, d) { // 外部调用缩放方法
                this.editingPanel.zoom(s, p, d);
            }, t.prototype.setGrid = function (gridLine) {
                ...
                var t = this;
                return new pt(new rt({index: this.printPanels.length, paperType: "A4" }), t);
                ...
                try {
                  if (t && "object" == _typeof(t) && t.panels.length > 0) {
                    ...
                    t.panels.forEach(function(panel, index) {
                      if (index > curLen) {
                        e.printPanels.push(new pt(panel, e));
                ...
                function renderLayer(type) {
                    ...
                    elements.forEach(function (el, idx) {
                        ...
                        var itemDom = $(`<div class="layer-item" data-seq="${idx}">
                                            ...
                                            ${field ? `<span class="field" title="字段: ${field}">${field}</span>` : ''}
                    ...
                    // 拖拽
                    layerHead.off("mousedown").on("mousedown", function (e) {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    function showOption(elItemObj) {
                        ...
                        requestAnimationFrame(() => { 
                            ...
                            const elOptionHeight = elOption.outerHeight();
                            const rectOption = elOption[0].getBoundingClientRect();
                            const optionBottom = rectOption.top + elOptionHeight;
                            if (optionBottom > windowHeight) {
                                const margin = 20;
                                const newTop = windowHeight - elOptionHeight - margin;
                                elOption.css({ top: newTop });
                            }
                        });
                        ...
                        elOption.off("click", ".closed").on("click", ".closed", function (e) {
                            e.stopPropagation();
                            e.preventDefault();
                            elOption.hide();
