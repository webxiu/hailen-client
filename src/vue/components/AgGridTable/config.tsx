...
import { BlendedSearchProps } from "@/vue/components/BlendedSearch/index.vue";
import { ButtonListProps } from "@/vue/components/ButtonList/index.vue";
...
export function getAgGridColumns<T extends Record<string, any>>(columnDefOption: ColumnDefsOption<T>, config?: Record<string, RendererType>) {
  ...
  _columnDefs.forEach((item: any) => {
    ...
    } else if (config && config[item.field]) {
      // 处理自定义渲染单元格==
      const _render = config[item.field] as any;
      item.cellRenderer = defineComponent({
        render: ({ params }) => {
